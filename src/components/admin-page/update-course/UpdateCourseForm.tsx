  ...
  .superRefine((data, ctx) => {
    ...
    if (data.type === SubTopicType.CODING_QUESTION) {
      if (data.questionSource === 'LEETCODE' && !data.questionNumber) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Question number is required for LeetCode questions.',
          path: ['questionNumber'],
        });
      } else if (data.questionSource === 'MANUAL' && !data.manualQuestion) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          message: 'Question markdown is required for manual questions.',
          path: ['manualQuestion'],
        });
      }
    }
    if (data.type === SubTopicType.PROJECT && !data.projectMarkdown) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Project description is required for project subtopics.',
    ...
    if (
      ...
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: 'Content description is required for offline subtopics.',
...
export function UpdateCourseForm({ course }: UpdateCourseFormProps) {
    ...
    defaultValues: {
      ...
      topics: course.topics.map((topic) => ({
        ...
        subTopics: topic.subTopics.map((subTopic) => {
          let parsedTestCases = [];
          if (subTopic.testCases && typeof subTopic.testCases === 'string') {
            try {
              parsedTestCases = JSON.parse(subTopic.testCases);
            } catch (e) {
              console.error('Failed to parse test cases from DB:', e);
            }
          ...
          return {
            ...
            image: subTopic.imageUrl ? [{ name: subTopic.imageUrl }] : [],
            video: subTopic.videoUrl ? [{ name: subTopic.videoUrl }] : [],
            questionSource: subTopic.questionSource || QuestionSource.LEETCODE,
            ...
            offlineContentMarkdown: subTopic.offlineContentMarkdown || '',
          };
        }),
      })),
    },
  ...
  const { totalLoaded, totalProgressPercentage } = useMemo(() => {
    const loaded = Object.values(progress).reduce((acc, val) => acc + val, 0);
  ...
  const uploadFile = async (file: File, folder: string) => {
    ...
    if (!success)
      throw new Error(`Failed to get presigned URL: ${error?.toString()}`);
            ...
            topic.subTopics.map(async (subTopic) => {
              ...
              const videoImageUrl =
                subTopic.image?.[0] instanceof File
                  ? await uploadFile(subTopic.image[0], `${folderName}/images`)
                  : originalSubTopic?.imageUrl;

              const videoUrl =
                subTopic.video?.[0] instanceof File
                  ? await uploadFile(subTopic.video[0], `${folderName}/videos`)
              ...
              return {
                id: subTopic.id,
                title: question_object ? question_object.title : subTopic.title,
                ...
                questionHTML:
                  subTopic.questionSource === 'LEETCODE' && question_object
        ...
        <div className="flex h-screen">
          ...
          <main className="flex-1 overflow-y-auto p-4 md:p-8">
            ...
            >
              <Card
                id="course-details"
                // Changed: Added a slightly different background for the main card
                className="bg-slate-50 dark:bg-slate-900/50 border border-primary/20"
              >
                <CardHeader>
              ...
              <div className="space-y-4">
                ...
                {topicFields.map((topic, topicIndex) => (
                  <React.Fragment key={topic.id}>
                    <Card
                      // Changed: Used a different background for topic cards to create depth
                      className="border-dashed bg-slate-100 dark:bg-slate-800/40 border border-primary/20"
                      id={`topic-${topicIndex}`}
                    >
                      <CardHeader className="flex flex-row items-center justify-between">
                        <CardTitle>Topic {topicIndex + 1}</CardTitle>
                        <Button
                          type="button"
                          variant="ghost"
                          size="icon"
                      ...
                      <CardContent className="space-y-6">
                        ...
                        />
                        <Separator />
                        <SubTopicsFieldArray topicIndex={topicIndex} />
                      </CardContent>
                    </Card>
                    <div className="flex justify-center">
                      <Button
                        type="button"
                        // Changed: Consistent variant with create form
                        variant="ghost"
                        size="sm"
                        onClick={() =>
                ...
                >
                  <PlusCircle className="mr-2 h-4 w-4 text-primary" /> Add Topic
                  to End
                </Button>
                <FormMessage>
...
const SubTopicItem = ({ topicIndex, subTopicIndex, removeSubTopic }: any) => {
  ...
  return (
    <div
      // Changed: A lighter background for sub-topic items to make them stand out
      className="p-4 border rounded-md space-y-4 relative z-0 bg-background border-primary/10"
    ...
    >
            ...
            <FormControl>
              ...
              >
                <FormItem className="flex items-center space-x-3 space-y-0">
                  <FormControl>
                    <RadioGroupItem value={SubTopicType.VIDEO} />
                  </FormControl>
                  <FormLabel className="font-normal">Video</FormLabel>
                </FormItem>
                <FormItem className="flex items-center space-x-3 space-y-0">
                  <FormControl>
                    <RadioGroupItem value={SubTopicType.CODING_QUESTION} />
                  </FormControl>
                  <FormLabel className="font-normal">Coding Question</FormLabel>
                </FormItem>
                <FormItem className="flex items-center space-x-3 space-y-0">
                  <FormControl>
                    <RadioGroupItem value={SubTopicType.PROJECT} />
                  </FormControl>
                  <FormLabel className="font-normal">Project</FormLabel>
                </FormItem>
                <FormItem className="flex items-center space-x-3 space-y-0">
                  <FormControl>
                    <RadioGroupItem value={SubTopicType.OFFLINE_CONTENT} />
                  </FormControl>
                  <FormLabel className="font-normal">Offline Content</FormLabel>
                </FormItem>
              </RadioGroup>
            </FormControl>
            <FormMessage />
          ...
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
              <div className="space-y-0.5">
                ...
                <FormDescription>
                  Switch between LeetCode and Manual question input.
                </FormDescription>
              </div>
              <FormControl>
                <div className="flex items-center space-x-2">
                  ...
                  <Switch
                    ...
                    onCheckedChange={(checked) =>
                      field.onChange(checked ? 'MANUAL' : 'LEETCODE')
                    }
                  />
                  <Label>Manual</Label>
                </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              {fields.map((field, index) => (
                <div
                  key={field.id}
                  // Changed: Another background shade for the deepest level of nesting
                  className="p-4 border rounded-md space-y-4 relative mb-4 bg-slate-50 dark:bg-slate-800/50 border-primary/10"
                >
                  <FormField
                    control={control}
                  ...
                  <Button
                    type="button"
                    // Changed: A less "loud" destructive button
                    variant="outline"
                    size="sm"
              ...
              >
                <PlusCircle className="mr-2 h-4 w-4 text-primary" /> Add Test
                Case
              </Button>
            </div>
          </div>
      ...
      >
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel>Project Description (Markdown)</FormLabel>
              <FormControl>
                <Textarea className="resize-y" {...field} />
              </FormControl>
              <FormMessage />
...
const SubTopicsFieldArray = ({ topicIndex }: { topicIndex: number }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {subTopicFields.map((subTopic, subTopicIndex) => (
        <React.Fragment key={subTopic.id}>
          ...
          <div className="flex justify-center mt-2">
            ...
            >
              <PlusCircle className="mr-2 h-4 w-4 text-primary" /> Insert
              Subtopic Below
            </Button>
          </div>
        </React.Fragment>
      ...
      >
        <PlusCircle className="mr-2 h-4 w-4 text-primary" /> Add Subtopic to End
      </Button>
      <FormMessage>
        {
          control.getFieldState(`topics.${topicIndex}.subTopics`)?.error?.root
            ?.message
        }
      </FormMessage>
    </div>
