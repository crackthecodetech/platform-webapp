...
const VideoPlayer = dynamic(() => import('./VideoPlayer'), {
  ssr: false,
  loading: () => <div className="aspect-video w-full bg-black animate-pulse" />,
});
...
const CourseDisplayClient = ({
  ...
  const renderSubTopicIcon = (type: SubTopicType) => {
    switch (type) {
      ...
      case SubTopicType.CODING_QUESTION:
        return <Code className="mr-3 h-5 w-5 text-gray-500 flex-shrink-0" />;
      ...
      <div className="relative aspect-video rounded-lg overflow-hidden border bg-background mb-6 flex-shrink-0">
        ...
        {(activeSubtopic?.type === SubTopicType.CODING_QUESTION ||
          ...
          <div className="bg-gray-50 dark:bg-gray-900 text-foreground h-full overflow-y-auto p-6">
            <h2 className="text-2xl font-bold mb-4">{activeSubtopic.title}</h2>
                ...
                <>
                  ...
                  <div className="mt-6">
                    ...
                    <div className="space-y-4">
                      {getTestCases(activeSubtopic.testCases).map(
                        (tc, index) => (
                          <div
                            key={index}
  ...
  const SidebarContent = (
    <aside className="h-full p-4 border-l overflow-y-auto bg-white dark:bg-black">
      <h2 className="text-lg font-semibold mb-4">{totalSubtopics} Lessons</h2>
      <Accordion
        type="multiple"
              ...
              <ul className="space-y-1 pl-4">
                {topic.subTopics.map((subTopic) => (
                  ...
                  >
                    ...
                    <span className="flex-grow text-sm">{subTopic.title}</span>
                  </li>
                ))}
              </ul>
            </AccordionContent>
